...
```bash
curl -X POST https://gateway.pipeworx.io/v1/tools/unesco_uis_get_data \
  -H 'Content-Type: application/json' \
  -d '{"indicator":"CR.1","geoUnit":"BRA","start":2015,"end":2020}'
```

No account needed for the first calls. Inspect any tool: `GET https://gateway.pipeworx.io/v1/tools/unesco_uis_get_data`. Find one: `POST https://gateway.pipeworx.io/v1/tools/search_packs` with `{"query":"..."}`.
...
```json
{
  "mcpServers": {
    "unesco-uis": {
      "command": "npx",
      "args": ["-y", "@pipeworx/mcp-unesco-uis"]
    }
  }
}
```
